...
| 찾아! | `ShellGame` | 컵 탭 · 1 2 3 | 공을 0.6s 보여주고 컵을 덮은 뒤 2→3→4번(배율) 섞는다. **덮이는 순간 공은 화면에서 사라지고**(가려지기를 기대하지 않는다 — 화면 비율에 따라 삐져나올 수 있다) 컵이 들릴 때 다시 나타난다. 공이 든 컵이면 성공, 아니면 즉시 실패 |
